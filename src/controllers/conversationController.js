...
async function handleCasualConversation(message, userContext = {}) {
  try {
    ...
    // Use unified intelligence for sophisticated responses
    const intent = { intent: 'casual_chat', confidence: 0.9 };
    const response = await UnifiedIntelligenceService.generateResponse(
      message,
      intent,
      userContext.whatsappNumber || 'unknown'
    );

    if (response && response !== "I'm having trouble understanding. Can you try again?") {
      return response;
    }
