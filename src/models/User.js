...
async function linkWhatsAppToUser(email, whatsappNumber) {
  try {
    ...
    const numberExists = existingNumbers.some(
      (num) => num && num.replace(/[^\d]/g, '') === cleanNewNumber
    );
    ...
    const result = await db.collection('users').updateOne(
      { 'basicProfile.email': email.toLowerCase().trim() },
      {
        $set: {
          ...
          whatsappNumber, // Keep primary for backward compatibility
...
function getIncompleteFields(user) {
  ...
  // Check arrays
  if (
    !enhanced.yatraImpact ||
    !Array.isArray(enhanced.yatraImpact) ||
  ...
  if (
    !enhanced.communityAsks ||
    !Array.isArray(enhanced.communityAsks) ||
  ...
  if (
    !enhanced.communityGives ||
    !Array.isArray(enhanced.communityGives) ||
...
async function linkAdditionalEmail(whatsappNumber, newEmail) {
  try {
    ...
    if (existingUser) {
      const existingUserNumbers =
        existingUser.whatsappNumbers || [existingUser.whatsappNumber].filter(Boolean) || [];
      const isCurrentUser = existingUserNumbers.some(
        (num) => num && num.replace(/[^\d]/g, '') === cleanNumber
      );
...
async function getProfileCompletionStats() {
  try {
    ...
    Object.keys(fieldCount).forEach((field) => {
      stats.byField[field] =
        stats.total > 0 ? Math.round((fieldCount[field] / stats.total) * 100) : 0;
    });
