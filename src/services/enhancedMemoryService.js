...
class EnhancedMemoryService {
    ...
    static async detectFollowUp(session, userMessage, context) {
        ...
        const followUpPhrases = [
            'any more', 'more', 'another', 'other', 'else', 'similar',
            'like this', 'same', 'also', 'additionally', 'besides',
            'what about', 'how about', 'show more', 'next', 'different'
        ];
    ...
    static extractInterests(session, query) {
        ...
        // Domain keywords
        const domains = ['tech', 'finance', 'healthcare', 'education', 'marketing', 
                        'sales', 'design', 'engineering', 'consulting', 'startup'];
        
        // Skill keywords
        const skills = ['developer', 'designer', 'manager', 'analyst', 'consultant',
                       'engineer', 'architect', 'specialist', 'expert', 'lead'];
        ...
        skills.forEach(skill => {
            if (queryLower.includes(skill)) {
                session.profileInterests.skills[skill] = 
                    (session.profileInterests.skills[skill] || 0) + 1;
            }
        });
        ...
        try {
            ...
            if (context.lastSearch.includes('developer')) {
                ...
                suggestions.push(
                    'startup mentors',
                    'angel investors',
                    'co-founders looking for partners'
                );
            }
    ...
    static async getUserAnalytics(whatsappNumber) {
        try {
            ...
            return {
                ...
                followUpRate: session.behaviorMetrics.totalSearches > 0 
                    ? (session.behaviorMetrics.followUpSearches / session.behaviorMetrics.totalSearches * 100).toFixed(2) + '%'
                    : '0%',
