...
class UserMemoryService {
    ...
    static async getUserInsights(whatsappNumber) {
        try {
            ...
            const avgMessageLength = memory.conversations.length > 0
                ? memory.conversations.reduce((sum, conv) => sum + conv.userMessage.length, 0) / memory.conversations.length
                : 0;
    ...
    static async analyzeUserBehavior(whatsappNumber) {
        try {
            ...
            // Analyze intents
            const intents = memory.conversations.map(c => c.context.intent).filter(i => i !== 'unknown');
            const primaryIntent = this.getMostFrequent(intents);
            
            // Communication style
            const avgLength = memory.conversations.reduce((sum, c) => sum + c.userMessage.length, 0) / memory.conversations.length;
            const communicationStyle = avgLength < 50 ? 'concise' : avgLength < 150 ? 'moderate' : 'detailed';
            
            return {
                mostActiveHour,
                primaryIntent,
                communicationStyle,
                engagementLevel: memory.insights.totalInteractions > 50 ? 'high' : memory.insights.totalInteractions > 20 ? 'medium' : 'low',
