...
async function updateUserStats(whatsappNumber, activityType) {
  try {
    ...
    await db.collection('user_stats').updateOne({ whatsappNumber, createdAt: today }, statsUpdate, {
      upsert: true,
    });
  } catch (error) {
...
async function getAnalyticsDashboard(timeframe = '24h') {
  try {
    ...
    const [userActivity, searchAnalytics, systemMetrics, errorAnalytics, topQueries, userGrowth] =
      await Promise.all([
...
async function getSearchAnalyticsMetrics(timeRanges) {
  try {
    ...
    return {
      ...
      zeroResultRate:
        data.totalSearches > 0
          ? Math.round((data.zeroResultSearches / data.totalSearches) * 100)
          : 0,
    };
  } catch (error) {
...
async function getErrorAnalytics(timeRanges) {
  try {
    ...
    return {
      totalErrors:
        queryErrors.reduce((sum, err) => sum + err.count, 0) +
