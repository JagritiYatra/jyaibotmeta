...
class ContextAwareSearchService {
    ...
    static async processContextualSearch(whatsappNumber, userMessage, intent) {
        try {
            ...
            if (isFollowUp) {
                ...
                const followUpData = await this.processFollowUpSearch(
                    userMessage, 
                    context, 
                    whatsappNumber
                );
    ...
    static async isFollowUpQuery(message, context) {
        ...
        const followUpIndicators = [
            'more', 'another', 'any more', 'else', 'other',
            'similar', 'like this', 'same kind', 'also',
            'what about', 'how about', 'show me more',
            'different', 'besides', 'additional', 'next'
        ];
    ...
    static async processFollowUpSearch(userMessage, context, whatsappNumber) {
        ...
        let refinements = [];
        let excludeProfiles = context.lastResults || [];
