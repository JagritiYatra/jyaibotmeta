    ...
    const result = await db.collection('sessions').replaceOne(
      { whatsappNumber },
      {
        ...
        createdAt: sessionData.conversation_start
          ? new Date(sessionData.conversation_start)
          : new Date(),
        version: config.bot.version,
        metadata: {
...
async function loadUserSession(whatsappNumber) {
  try {
    ...
    if (!session) {
      logSuccess('new_session_created', {
        whatsappNumber: whatsappNumber.replace(/[^\d]/g, '').slice(-4),
      });
    ...
    await db.collection('sessions').updateOne({ whatsappNumber }, { $set: { lastActivity: now } });
...
async function updateSessionState(whatsappNumber, stateUpdates) {
  try {
    ...
    const result = await db
      .collection('sessions')
      .updateOne({ whatsappNumber }, { $set: updateFields });
...
async function getSessionStats() {
  try {
    ...
    const [totalSessions, activeSessions, recentSessions, authenticatedSessions] =
      await Promise.all([
...
function validateSessionData(sessionData) {
  ...
  const validStates = [
    ...
    'updating_fullName',
    'updating_gender',
    'updating_professionalRole',
    'updating_dateOfBirth',
    'updating_country',
    'updating_city',
    'updating_state',
    'updating_phone',
    'updating_additionalEmail',
    'updating_linkedin',
    'updating_instagram',
    'updating_domain',
    'updating_yatraImpact',
    'updating_communityAsks',
    'updating_communityGives',
  ];
