...
async function sendTwilioMessage(whatsappNumber, messageText, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.error(
        `❌ Twilio attempt ${attempt} failed for ***${sanitizedNumber}:`,
        error.message
      );
...
async function sendBulkMessages(recipients, messageText, options = {}) {
  ...
  for (let i = 0; i < recipients.length; i += batchSize) {
    ...
    console.log(
      `📊 Batch ${Math.floor(i / batchSize) + 1} completed: ${batchResults.filter((r) => r.success).length}/${batchResults.length} successful`
    );
  }
