...
router.get('/users/:whatsappNumber', asyncHandler(async (req, res) => {
    try {
        ...
        logError(error, { operation: 'admin_user_details', whatsappNumber: req.params.whatsappNumber });
        res.status(500).json({ error: 'Failed to fetch user details' });
    }
}));

// Update user profile
router.put('/users/:whatsappNumber', asyncHandler(async (req, res) => {
    try {
        const { whatsappNumber } = req.params;
        ...
        logError(error, { operation: 'admin_user_update', whatsappNumber: req.params.whatsappNumber });
        res.status(500).json({ error: 'Failed to update user' });
    }
}));

// Delete user
router.delete('/users/:whatsappNumber', asyncHandler(async (req, res) => {
    try {
        const { whatsappNumber } = req.params;
        ...
        const sessionDir = path.join(__dirname, '../../sessions/memory', whatsappNumber.replace(/[^\d]/g, ''));
        try {
        ...
        logError(error, { operation: 'admin_user_delete', whatsappNumber: req.params.whatsappNumber });
...
router.get('/conversations/:whatsappNumber', asyncHandler(async (req, res) => {
    try {
        const { whatsappNumber } = req.params;
        ...
        const allConversations = [
            ...conversations,
            ...enhancedConversations.filter(ec => 
                !conversations.find(c => c.timestamp === ec.timestamp)
            )
        ...
        logError(error, { operation: 'admin_conversations', whatsappNumber: req.params.whatsappNumber });
        res.status(500).json({ error: 'Failed to fetch conversations' });
    }
}));

// Search analytics - Get real search data from queries collection
router.get('/analytics/searches', asyncHandler(async (req, res) => {
    try {
        const db = getDatabase();
        
        // Get search queries from queries collection
        const searchQueries = await db.collection('queries')
            .find({ 
        ...
        searchQueries.forEach(queryLog => {
            ...
            if (query.includes('developer') || query.includes('engineer') || 
                query.includes('tech') || query.includes('programming')) {
                searchCategories['Technical'].push(query);
            } else if (query.includes('marketing') || query.includes('sales') || 
                       query.includes('business') || query.includes('finance')) {
                searchCategories['Business'].push(query);
            } else if (query.includes('mumbai') || query.includes('delhi') || 
                       query.includes('bangalore') || query.includes('pune')) {
                searchCategories['Location'].push(query);
            } else if (query.includes('skill') || query.includes('expert') || 
        ...
        const dailySearches = await db.collection('queries')
            .aggregate([
                {
                    $match: {
                        queryType: { $in: ['search', 'alumni_search'] },
