...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    console.log(
      `📱 Processing message from ${whatsappNumber.replace(/[^\d]/g, '').slice(-4)} (${userName}): ${userMessage.substring(0, 100)}${userMessage.length > 100 ? '...' : ''}`
    );
    ...
    try {
      ...
      if (!userSession) {
        console.log(
          `🆕 Creating new session for ${whatsappNumber.replace(/[^\d]/g, '').slice(-4)}`
        );
        userSession = createEmergencySession();
      }

      // Detect user intent using unified intelligence AI system
      const aiIntent = await UnifiedIntelligenceService.analyzeMessage(userMessage, whatsappNumber);
      ...
      const rawIntent =
        aiIntent.intent !== 'unknown'
          ...
          : detectUserIntent(userMessage, userSession);
      console.log(`🧠 Final intent: ${rawIntent.type} (confidence: ${rawIntent.confidence})`);
      ...
      if (!responseMessage || responseMessage.trim().length === 0) {
        console.log(
          `⚠️ Empty response generated for ${whatsappNumber.replace(/[^\d]/g, '').slice(-4)}`
        );
      ...
      if (!sessionSaved) {
        console.log(
          `⚠️ Failed to save session for ${whatsappNumber.replace(/[^\d]/g, '').slice(-4)}`
        );
      }
      ...
      if (responseMessage) {
        ...
        if (messageSent && messageSent.success) {
          console.log(
            `✅ Response sent successfully to ${whatsappNumber.replace(/[^\d]/g, '').slice(-4)}`
          );

          // Track conversation in MongoDB memory
          await MongoMemoryService.addConversation(whatsappNumber, userMessage, responseMessage, {
            intent: intent.type,
            topic: intent.query || intent.type,
            searchQuery: intent.query,
            isFollowUp:
              intent.type === 'follow_up' ||
              (await MongoMemoryService.isFollowUp(whatsappNumber, userMessage)),
          });
          ...
          console.log(
            `❌ Failed to send response to ${whatsappNumber.replace(/[^\d]/g, '').slice(-4)}: ${messageSent?.error || 'Unknown error'}`
          );
          ...
          try {
            await sendTwilioMessage(From, '⚠️ Message delivery failed. Please try again.', {
              maxRetries: 1,
            });
          } catch (fallbackError) {
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      const healthStatus = {
        ...
        twilio:
          config.twilio.accountSid && config.twilio.authToken ? 'configured' : 'not_configured',
